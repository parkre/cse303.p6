...
void file_server(int connfd, int lru_size){
    ...
    if(is_put){
        ...
        unsigned char planned_md5[md5_size];
        if(Receive(connfd, planned_md5, md5_size) != 0){
        ...
        MD5(file, sizeof(file), actual_md5);
        /* validate MD5's */
        fprintf(stderr, "Planned MD5: %u\nActual MD5: %u\n", planned_md5, actual_md5);
        if(memcmp(planned_md5, actual_md5, sizeof(actual_md5)) != 0){
    ...
    /* handle GET */
        ...
        uint32_t response_headersize = strlen("OK") + strlen(filename) + strlen(size) + 4;  
        ...
        MD5(file, sizeof(file), md5_hash);
        fprintf(stderr, "MD5 for GET: %u\n", md5_hash);
        /* send the MD5 hash */
        Send_Int(connfd, sizeof(md5_hash));
        Send(connfd, md5_hash, sizeof(md5_hash));
        /* send the file */
